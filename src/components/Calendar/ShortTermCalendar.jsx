...
const ZOOM_LEVELS = [
  { label: '⅛x',  hourHeight: 8,    hourFocus: false },  // Full day in ~192px — ultra overview
  { label: '¼x',  hourHeight: 14,   hourFocus: false },  // Full day in ~336px — compact overview
...
export default function ShortTermCalendar() {
  ...
  const handleTaskDoubleClick = (task, e) => {
    ...
    const targetLevel = Math.max(zoomLevel, 10); // At least 12x
  ...
  const renderTimeGrid = (dates, showTimeColumn = true) => {
    ...
    return (
      <div className={`time-grid-container ${isHourFocus ? 'hour-focus-mode' : ''} ${HOUR_HEIGHT <= 14 ? 'ultra-micro-zoom micro-zoom' : HOUR_HEIGHT <= 30 ? 'micro-zoom' : HOUR_HEIGHT < 60 ? 'compact-zoom' : ''} ${dragState.active ? 'is-dragging' : ''} ${dragState.active && dragState.type === 'move' ? 'is-dragging-move' : ''}`} ref={calendarRef} style={isHourFocus ? { overflow: 'hidden' } : {}}>
          ...
          {showTimeColumn && (
            <div className="time-labels">
              {hoursToRender.map((hour) => {
                // At micro zoom, only show every Nth label to prevent overlap
                const showLabel = isHourFocus ? true
                  : HOUR_HEIGHT <= 8 ? (hour % 6 === 0)
                  : HOUR_HEIGHT <= 14 ? (hour % 4 === 0)
              ...
              <div key={date.toISOString()} className="day-column" data-date={format(date, 'yyyy-MM-dd')}>
                ...
                >
                  ...
                  {(() => {
                    ...
                    const isUltraMicroZoom = HOUR_HEIGHT <= 14;
                    ...
                    return (
                      <div
                        key={task.id}
                        className={`calendar-task ${!customColor ? `priority-${task.priority}` : ''} type-${task.type} ${task.completed ? 'completed' : ''} ${isZoomedIn ? 'zoomed' : ''} ${isDeepZoom ? 'deep-zoom' : ''} ${isUltraZoom ? 'ultra-zoom' : ''} ${isUltraMicroZoom ? 'ultra-micro-zoom micro-zoom' : isMicroZoom ? 'micro-zoom' : isCompactZoom ? 'compact-zoom' : ''} ${customColor ? 'custom-color' : ''} ${durationCls} ${compactMode ? 'compact' : ''} ${task._segment === 'start' ? 'segment-start' : ''} ${task._segment === 'continuation' ? 'segment-continuation' : ''}`}
                        style={colorStyle}
